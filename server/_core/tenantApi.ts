...
const requireApiKey = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!validApiKey) {
    return res
      .status(500)
      .json({ error: "Platform API key is not configured" });
  }
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      if (existing) {
        return res
          .status(409)
          .json({ success: false, error: "Slug already in use" });
      }
      ...
      if (existingUser) {
        return res
          .status(409)
          .json({ success: false, error: "Admin email already registered" });
      }
      ...
      try {
        ...
        if (tenantConfig) {
          ...
          if (tenantDb) {
            ...
            console.error(
              `[TenantApi] Failed to connect to DB for seeding templates: ${data.slug}`
            );
          }
        }
      } catch (seedError) {
        console.error(
          `[TenantApi] Error seeding email templates for ${data.slug}:`,
          seedError
        );
      }
      ...
      res
        .status(201)
        .json({ success: true, data: { id: tenantId, slug: data.slug } });
    } catch (error: any) {
