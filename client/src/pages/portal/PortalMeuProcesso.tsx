      ...
      <div className="relative">
        ...
        <div className="space-y-4">
          {steps.map((step, idx) => {
            ...
            const isCurrent =
              !step.completed && (idx === 0 || steps[idx - 1]?.completed);
            const subTasks =
              typeof step.subTasks === "string"
                ? JSON.parse(step.subTasks)
                : step.subTasks || [];
            ...
            return (
              <div key={step.id} className="relative flex gap-4 z-10">
                ...
                {/* Card da etapa */}
                <div
                  className={`flex-1 rounded-xl border shadow-sm overflow-hidden mb-1
                ...
                >
                  ...
                  >
                    <div>
                      <p
                        className={`font-semibold text-sm ${step.completed ? "text-green-800" : isCurrent ? "text-purple-800" : "text-gray-700"}`}
                      >
                        {step.stepTitle}
                      </p>
                      ...
                      {step.stepId === "acompanhamento-sinarm" &&
                        step.sinarmStatus && (
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full font-medium ${SINARM_STATUS_LABELS[step.sinarmStatus]?.color || "bg-gray-100 text-gray-700"}`}
                            >
                              {SINARM_STATUS_LABELS[step.sinarmStatus]?.label ||
                                step.sinarmStatus}
                            </span>
                            {step.protocolNumber && (
                              <span className="text-xs text-gray-500">
                                Protocolo: {step.protocolNumber}
                              </span>
                            )}
                          </div>
                        )}
                      ...
                      {subTasks.length > 0 && (
                        <div>
                          <p className="text-xs font-semibold text-gray-600 mb-2">
                            Itens (
                            {subTasks.filter((s: any) => s.completed).length}/
                            {subTasks.length} concluídos):
                          </p>
