...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  active: { label: "Ativa", color: "bg-green-100 text-green-700", icon: <CheckCircle2 className="h-3.5 w-3.5" /> },
  trialing: { label: "Trial", color: "bg-blue-100 text-blue-700", icon: <Clock className="h-3.5 w-3.5" /> },
  past_due: { label: "Em Atraso", color: "bg-orange-100 text-orange-700", icon: <AlertCircle className="h-3.5 w-3.5" /> },
  cancelled: { label: "Cancelada", color: "bg-red-100 text-red-700", icon: <XCircle className="h-3.5 w-3.5" /> },
  expired: { label: "Expirada", color: "bg-gray-100 text-gray-600", icon: <XCircle className="h-3.5 w-3.5" /> },
  pending: { label: "Pendente", color: "bg-yellow-100 text-yellow-700", icon: <Clock className="h-3.5 w-3.5" /> },
  paid: { label: "Paga", color: "bg-green-100 text-green-700", icon: <CheckCircle2 className="h-3.5 w-3.5" /> },
  overdue: { label: "Vencida", color: "bg-red-100 text-red-700", icon: <AlertCircle className="h-3.5 w-3.5" /> },
};
...
function UsageBar({ label, current, max, unit = "" }: { label: string; current: number; max: number; unit?: string }) {
  ...
  const color = pct >= 90 ? "bg-red-500" : pct >= 75 ? "bg-orange-500" : "bg-indigo-500";
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-sm">
        <span className="text-gray-700">{label}</span>
        <span className="text-gray-500 font-mono">{current}{unit} / {max}{unit}</span>
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={() => setShowPlanChange(true)}>
              Alterar Plano
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-3">
          {activeSub ? (
            <>
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-gray-900">Plan ID #{activeSub.planId}</p>
                  <p className="text-sm text-gray-500">
                    {activeSub.billingCycle === "monthly" ? "Mensal" : activeSub.billingCycle === "yearly" ? "Anual" : "Vitalício"}
                    {" · "}{formatBRL(activeSub.priceBRL - activeSub.discountBRL)}/ciclo
                  </p>
                </div>
              ...
              {activeSub.status !== "cancelled" && (
                <Button
                  ...
                  onClick={() => {
                    if (confirm("Cancelar assinatura? O tenant será suspenso.")) {
                      cancelMutation.mutate({ id: activeSub.id, tenantId, reason: "Cancelada pelo admin" });
                    }
                  }}
                >
                  {cancelMutation.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" /> : null}
                  Cancelar Assinatura
                </Button>
              )}
            </>
            ...
            <div className="text-center py-4 text-gray-500">
              ...
              <Button size="sm" className="mt-3 bg-indigo-600 hover:bg-indigo-700" onClick={() => setShowPlanChange(true)}>
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={() => setShowInvoiceCreate(true)}>
        ...
        <CardContent>
          {invLoading ? (
            ...
            <p className="text-sm text-gray-500 text-center py-4">Nenhuma fatura gerada</p>
          ) : (
            <div className="space-y-2">
              {invoices.slice(0, 10).map((inv: any) => (
                <div key={inv.id} className="flex items-center justify-between p-3 rounded-lg border bg-white hover:bg-gray-50 transition-colors">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">
                      {formatDate(inv.periodStart)} – {formatDate(inv.periodEnd)}
                    </p>
                    <p className="text-xs text-gray-500">
                      Venc. {formatDate(inv.dueDate)} · {formatBRL(inv.totalBRL)}
                    </p>
                  </div>
              ...
              {invoices.length > 10 && (
                <p className="text-xs text-gray-400 text-center pt-1">+ {invoices.length - 10} faturas anteriores</p>
              )}
            </div>
          )}
        </CardContent>
      ...
      {subs.length > 1 && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base text-gray-700">Histórico de Assinaturas</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              {subs.slice(0, 5).map((sub: any) => (
                <div key={sub.id} className="flex items-center justify-between py-2 border-b last:border-0">
                  <div className="text-sm">
                    ...
                    <span className="text-gray-400 ml-2">{formatDate(sub.startDate)}</span>
                  </div>
