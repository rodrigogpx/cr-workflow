...
export const clients = pgTable(
  "clients",
  {
    ...
    hasSecondCollectionAddress: boolean("hasSecondCollectionAddress").default(
      false
    ),
...
export type InsertSinarmCommentHistory =
  typeof sinarmCommentsHistory.$inferInsert;

export const sinarmCommentsHistoryRelations = relations(
...
export const emailTriggers = pgTable("emailTriggers", {
  ...
  // Destinatários
  recipientType: varchar("recipientType", { length: 20 })
    .notNull()
    .default("client"), // 'client', 'users', 'both', 'operator'
...
export type InsertEmailTriggerTemplate =
  typeof emailTriggerTemplates.$inferInsert;

export const emailTriggerTemplatesRelations = relations(
...
export const emailScheduledRelations = relations(
  emailScheduled,
  ({ one }: any) => ({
    client: one(clients, {
...
export const tenants = pgTable("tenants", {
  ...
  emailMethod: varchar("emailMethod", { length: 20 })
    .default("gateway")
    .$type<"smtp" | "gateway">(), // smtp = direto, gateway = HTTP relay
  ...
  signatureResponsibleName: varchar("signatureResponsibleName", {
    length: 255,
  }), // Nome para aparecer na assinatura dos documentos
  // Storage
  storageBucket: varchar("storageBucket", { length: 255 }),
  backupSchedule: varchar("backupSchedule", { length: 50 }).default(
    "0 3 * * *"
  ),
  // Limits
  maxUsers: integer("maxUsers").default(10),
  maxClients: integer("maxClients").default(500),
  maxStorageGB: numeric("maxStorageGB", { precision: 10, scale: 2 }).default(
    "50"
  ),
  // Subscription
...
export const planDefinitions = pgTable("planDefinitions", {
  ...
  maxStorageGB: numeric("maxStorageGB", { precision: 10, scale: 2 })
    .notNull()
    .default("10"),
  featureWorkflowCR: boolean("featureWorkflowCR").notNull().default(true),
  featureApostilamento: boolean("featureApostilamento")
    .notNull()
    .default(false),
...
export const subscriptions = pgTable("subscriptions", {
  ...
  billingCycle: varchar("billingCycle", { length: 20 })
    .notNull()
    .default("monthly"),
  ...
  status: varchar("status", { length: 20 })
    .notNull()
    .default("active")
    .$type<"active" | "past_due" | "cancelled" | "expired" | "trialing">(),
...
export const invoices = pgTable("invoices", {
  ...
  status: varchar("status", { length: 20 })
    .notNull()
    .default("pending")
    .$type<"pending" | "paid" | "overdue" | "cancelled" | "refunded">(),
...
export const usageSnapshots = pgTable(
  "usageSnapshots",
  {
    ...
    storageUsedGB: numeric("storageUsedGB", { precision: 10, scale: 3 })
      .notNull()
      .default("0"),
    dbSizeMB: numeric("dbSizeMB", { precision: 10, scale: 1 })
      .notNull()
      .default("0"),
    createdAt: timestamp("createdAt", { withTimezone: true })
      .defaultNow()
  ...
  (table: any) => [
    unique("usageSnapshots_tenantId_snapshotDate_unique").on(
      table.tenantId,
      table.snapshotDate
    ),
  ]
...
export type InsertPlatformAdminAuditLog =
  typeof platformAdminAuditLogs.$inferInsert;
...
export const tenantActivityLogs = pgTable("tenantActivityLogs", {
  ...
  performedAt: timestamp("performedAt", { withTimezone: false })
    .defaultNow()
    .notNull(),
});
...
export const iatInstructorsRelations = relations(
  iatInstructors,
  ({ one, many }: any) => ({
    user: one(users, {
...
export const iatSchedulesRelations = relations(
  iatSchedules,
  ({ one }: any) => ({
...
export const iatCourseClassesRelations = relations(
  iatCourseClasses,
  ({ one, many }: any) => ({
    course: one(iatCourses, {
...
export const iatClassInstructors = pgTable("iat_class_instructors", {
  ...
  assignedAt: timestamp("assignedAt", { withTimezone: false })
    .defaultNow()
    .notNull(),
});
...
export const iatClassInstructorsRelations = relations(
  iatClassInstructors,
  ({ one }: any) => ({
...
export const iatClassEnrollments = pgTable("iat_class_enrollments", {
  ...
  enrolledAt: timestamp("enrolledAt", { withTimezone: false })
    .defaultNow()
    .notNull(),
  completedAt: timestamp("completedAt", { withTimezone: false }),
...
export const iatClassEnrollmentsRelations = relations(
  iatClassEnrollments,
  ({ one }: any) => ({
    courseClass: one(iatCourseClasses, {
...
export const iatClassSessionsRelations = relations(
  iatClassSessions,
  ({ one, many }: any) => ({
    courseClass: one(iatCourseClasses, {
...
export const iatAttendanceRelations = relations(
  iatAttendance,
  ({ one }: any) => ({
...
export const clientPendingDocuments = pgTable("clientPendingDocuments", {
  ...
  uploadedAt: timestamp("uploadedAt", { withTimezone: false })
    .defaultNow()
    .notNull(),
...
export type InsertClientPendingDocument =
  typeof clientPendingDocuments.$inferInsert;
